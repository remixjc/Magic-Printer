...
- 文档版本：1.0
- 基准日期：2026-08-07
- 估算口径：2 名全职开发（桌面/后端、前端）+ 兼职测试/产品；不依赖厂商 SDK 或授权接口。
...
- E-safe 采用“本地启发式识别 + 未知默认阻断”的明确降级范围。
- 对首期支持格式和系统版本完成冻结。
...
- E-safe 本地启发式 detector、特征规则和风险提示。
- 加密/疑似/不可用的阻断和提示流程。
...
- 本地启发式 E-safe 风险拦截经过样本验证，并明确不宣称官方 100% 识别。
- 安全的局域网模式。
